...
export async function removeTrackingNumber(id: string) {
  try {
    ...
    // Trigger storage event to update DateCounter
...
export async function addImageToTracking(id: string, imageData: string) {
  ...
  const updatedEntries = entries.map((entry) =>
    entry.id === id
      ? { ...entry, imageData: [...entry.imageData, imageData] }
      : entry
  );
...
export async function checkDuplicateTracking(
  trackingNumber: string
): Promise<TrackingEntry | null> {
  try {
    const response = await fetch(
      `/api/fedex-tracking/check-duplicate?trackingNumber=${encodeURIComponent(
        trackingNumber
      )}`,
      {
        method: "GET",
...
export function updateCustomStatusType(
  id: string,
  updatedStatusType: CustomStatusType
) {
  const statusTypes = getCustomStatusTypes();
  const updatedStatusTypes = statusTypes.map((statusType) =>
    statusType.id === id ? updatedStatusType : statusType
  );
